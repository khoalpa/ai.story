...
def _module_package_root(module_file: str | Path | None = None) -> Path:
    ...
    for candidate in (current, *current.parents):
        if (candidate / "_shared" / "runtime.py").is_file():
            return candidate
        if candidate.name == "_shared" and (candidate / "runtime.py").is_file():
            return candidate.parent
    return _shared_package_root().parent


def resolve_common_package_assets_root(module_file: str | Path | None = None) -> Path:
    return (_module_package_root(module_file) / "_shared" / "assets").resolve()


def _source_common_assets_root(path: Path) -> Path:
    return (_module_package_root(path) / "_shared" / "assets").resolve()
