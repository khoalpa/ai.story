...
from video.story_zone_timeline import StoryZoneSegment, estimate_story_zone_duration
...
def test_cover_replaces_start_of_regular_slideshow_without_extending_it(tmp_path: Path) -> None:
    ...
    assert estimate_story_zone_duration(result) == estimate_story_zone_duration(base) == 20.0


def test_cover_can_consume_multiple_short_segments_without_duplicates(tmp_path: Path) -> None:
    cover = tmp_path / "cover.png"
    base = [
        StoryZoneSegment("greeting", tmp_path / "greeting.png", 0.0, 1.0),
        StoryZoneSegment("opening", tmp_path / "opening.png", 1.0, 2.5),
        StoryZoneSegment("development", tmp_path / "development.png", 2.5, 8.0),
    ]
    ...
    assert estimate_story_zone_duration(result) == 8.0


def test_missing_cover_selection_keeps_existing_timeline(tmp_path: Path) -> None:
    base = [StoryZoneSegment("opening", tmp_path / "opening.png", 0.0, 5.0)]
