...
_SOURCE_MARKERS = ("pyproject.toml", "video")
_LEGACY_SOURCE_ASSETS_DIRNAME = "assets"
_SOURCE_COMMON_ASSETS_RELATIVE = Path("video") / "assets"
...
def resolve_package_assets_root(module_file: str | Path | None = None) -> Path:
    if module_file is not None:
        ...
        sibling_common_assets = (module_path.parent.parent / "video" / "assets").resolve()
...
def resolve_assets_root_for_module(module_file: str | Path, project_root: Path | None = None) -> Path:
    ...
    source_common_assets = _source_package_assets_root(project)
    ...
    bundled_assets = resolve_package_assets_root(module_file)
...
def compute_standard_paths(module_file: str | Path) -> dict[str, Path]:
    ...
    project_assets_root = _source_package_assets_root(project_root)
