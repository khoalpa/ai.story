...
def _fixture(root: Path) -> dict[str, object]:
    """Exercise the portable Audio-to-Video handoff without network credentials."""
    from PIL import Image

    from audio.app_api import write_video_handoff
    from video.app_api import read_audio_handoff
    ...
    Image.new("RGB", (360, 640), "navy").save(cover)
    Image.new("RGB", (360, 640), "teal").save(scenes / "01_opening.png")
    audio_manifest = write_video_handoff(root / "audio_video_handoff.json", audio=audio)
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Audio-to-Video E2E contract runner")
    ...
    if not args.fixture:
        result: dict[str, object] = {
            ...
            "reason": "Provider E2E requires explicit inputs; use --fixture for offline CI.",
            "stages": {name: not getattr(args, f"skip_{name}") for name in ("audio", "video")},
        }
    elif args.workspace:
        result = _fixture(args.workspace.resolve())
    else:
        with tempfile.TemporaryDirectory(prefix="ai-audio-video-e2e-") as temporary:
