...
_PACKAGE_ASSETS_DIRNAME = "assets"
...
def _module_package_root(module_file: str | Path | None = None) -> Path:
    ...
    for candidate in (current, *current.parents):
        if (candidate / "runtime.py").is_file() and (candidate / "assets").is_dir():
            return candidate
        if (candidate / "__init__.py").is_file() and candidate.name == "story":
            return candidate
...
def _legacy_source_assets_root(path: Path) -> Path:
    return (path / _PACKAGE_ASSETS_DIRNAME).resolve()
