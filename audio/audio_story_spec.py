...
# Contract governance versions
CANONICAL_CONTRACT_VERSION = "1.1.0"
PLAIN_SCRIPT_FORMAT_VERSION = "1.1.0"
...
def normalize_canonical_authoring_zones(data: Any) -> Any:
    ...
    if isinstance(outline, dict):
        ...
        for key, value in outline.items():
            canonical_key = normalize_outline_key(key)
            if canonical_key != key:
                outline_changed = True
            new_outline[canonical_key] = value
        if outline_changed:
            normalized["outline"] = new_outline
            changed = True
...
def build_meta_header(meta: dict) -> str:
    ...
    lines: List[str] = [f"# FORMAT: AUDIO_STORY_PLAIN v{PLAIN_SCRIPT_FORMAT_VERSION}", f"# CONTRACT_VERSION: {CANONICAL_CONTRACT_VERSION}"]
...
def validate_canonical_authoring(data: Any) -> List[str]:
    ...
    if not isinstance(outline, dict):
        ...
        normalized_outline_keys = [normalize_outline_key(k) for k in outline.keys()]
        extra_outline = sorted({str(k) for k, nk in zip(outline.keys(), normalized_outline_keys) if nk not in OUTLINE_KEYS})
        if extra_outline:
            errors.append(f"outline có field không hợp lệ: {extra_outline}.")
        if tuple(normalized_outline_keys) != OUTLINE_KEYS:
...
def render_canonical_script_items(script: object) -> str:
    ...
    - `zone`, `environment`, and `text` are required; `environment` may be "none" when no ambience hint is intended.
    ...
    for idx, item in enumerate(script, start=1):
        ...
        env_tag = "" if not environment or environment.lower() == "none" else f"[SFX={environment}]"
        out.append(f"[{voice}][{speed}][{lang}]{env_tag} {text}")
