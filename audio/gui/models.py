...
def render_models_tab(_settings: dict) -> None:
    """Inventory and safely maintain Audio's local model store."""
    st.subheader("Mô hình")
    root = models_root(__file__)
    st.caption(f"Kho model: {root}")
    controls = st.columns([1.2, 1.0, 1.0])
    include_cache = controls[0].checkbox("Bao gồm cache", value=False, key="audio_models_include_cache")
    max_depth = controls[1].number_input("Độ sâu", min_value=1, max_value=8, value=3, step=1, key="audio_models_depth")
    if controls[2].button("Làm mới", key="audio_models_refresh", width="stretch"):
        st.rerun()
    ...
    st.download_button("Tải báo cáo JSON", data=json.dumps(report.as_dict(), ensure_ascii=False, indent=2), file_name="audio_models_report.json", mime="application/json", key="audio_models_download", width="stretch")
    st.divider()
    left, right = st.columns(2)
    with left:
        prune_confirm = confirm_destructive_action(
            "Tôi xác nhận xóa các thư mục rỗng",
            key="audio_models_prune_confirm",
        )
        if st.button("Prune thư mục rỗng", key="audio_models_prune", disabled=not prune_confirm, width="stretch"):
            paths = prune_empty_model_directories(__file__, apply=True)
            st.success(f"Đã xoá {len(paths)} thư mục rỗng.")
            if paths:
                st.code("\n".join(paths))
    with right:
        remove_path = st.text_input("Đường dẫn cần xoá (tương đối)", key="audio_models_remove_path")
        remove_confirm = confirm_destructive_action(
            "Tôi xác nhận xóa mô hình đã chọn",
            key="audio_models_remove_confirm",
        )
        if st.button("Xoá model", key="audio_models_remove", disabled=not remove_path.strip() or not remove_confirm, width="stretch"):
            try:
